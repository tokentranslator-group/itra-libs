...
/*
define(['jquery', 'jquery-ui-custom/jquery-ui', 'tooltip', 'tinput'],
       
       function($, ui, tooltip, tinput){
*/
	   function Menu(m_id, input_id, items, tooltips, callbacks){
	       /* Manipulation with menu.
		      
		-- ``m_id`` - actual div id of index.htm,
		where menu will be stored.

		-- ``input_id`` - actual div id of index.htm,
		...
		as ``items``.
	       */
	       ...
	       self.input = new tinput.Input(self, self.input_div_id);
	       tooltip.init();
	       // END FOR
	   }
	   ...
	   Menu.prototype.menu_create = function(x, y){
		   ...
		   /*On right click. All callbacks will activate here.*/

		   console.log("select");
		   // console.log(event);
		   // console.log(ui);
		   // console.log(self.menu_callbacks);
		   if (self.menu_callbacks){
		       var idx = ui.item[0].textContent;
		       // console.log(ui.item[0].textContent);
		       self.menu_callbacks[idx]();
		   }else{
